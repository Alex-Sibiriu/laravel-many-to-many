    ...
    <div class="col-12">
      <div class="px-2 bg-dark rounded-3 pb-1">
        ...
        <p class="fs-4 ps-3 text-white"><strong>Parametro di ricerca: </strong><span
            class="badge fs-6 ms-1 text-bg-success">{{ $type->name }}</span></p>
                ...
                <td class="align-content-center text-center">
                  @forelse ($project->technologies as $technology)
                    <a class="text-decoration-none" href="{{ route('admin.technologies.show', $technology) }}">
                      <span class="badge fs-6 ms-1 text-bg-primary">{{ $technology->name }}</span>
                    </a>
                  @empty
                    <span class="badge fs-6 text-bg-warning">Nessuna Tecnologia Associata</span>
                  @endforelse
                </td>

                <td class="align-content-center text-center">
                  @if ($project->type)
                    <span class="badge text-bg-success text-decoration-none fs-6">
                      {{ $project->type->name }}
                    </span>
                  @else
                    ---
